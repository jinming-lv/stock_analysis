...
- [改进] `StockTrendAnalyzer` 支持显式传入乖离率阈值，使纯离线指标示例和测试无需加载完整运行配置或 `.env`，生产默认行为保持不变。
- [测试] 新增课程示例 smoke test，挨个儿执行 22 个脚本并校验完成标记。
- [文档] 新增 600519 脱敏真实数据、报告与接口样例说明，明确课程用途、数据日期和投资风险边界。
